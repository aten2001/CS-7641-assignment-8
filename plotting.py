...
def plot_best_curves(problem_name, files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        # swyx
        timedf = main_df['time'].drop(columns=['MIMIC_mean', 'MIMIC_std']).iloc[1:] # dont want to divide by 0
        timeprefixes = ['GA', 'RHC', 'SA']
        # print('xxxxxxxxx')
        # print(prefixes)
        # timedf['SA_mean'] = timedf['SA_mean'] / (timedf.index)
        # timedf['SA_std'] = timedf['SA_std'] / (timedf.index)
        # timedf['GA_mean'] = timedf['GA_mean'] / (timedf.index)
        # timedf['GA_std'] = timedf['GA_std'] / (timedf.index)
        # timedf['RHC_mean'] = timedf['RHC_mean'] / (timedf.index)
        # timedf['RHC_std'] = timedf['RHC_std'] / (timedf.index)
        # print(timedf.head())
        # print('xxxxxxxxx')
        p = plot_data('{} - Best: {} vs Iterations'.format(problem_name, 'Wall Time'), timedf,
                      timeprefixes, nn_curve=nn_curve, validate_only=nn_curve, # ylim=(0,2),
                      y_label='Wall Time')
        p.savefig(
